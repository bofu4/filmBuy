...
public class OrderInfoService {
    ...
    private void packOrder(OrderInfo orderInfo) {
        ...
        for (OrderGoodsRel rel : rels) {
            ...
            if (goodsDetailInfo != null) {
                // 注意这里返回的count是用户加入电影的数量，而不是电影的库存
    ...
    public PageInfo<OrderInfo> findPages(Long userId, Integer pageNum, Integer pageSize, HttpServletRequest request) {
        ...
        if (user == null) {
            throw new CustomException("1001", "session has expired, please log in again");
        }
		...
		if (level == 1) {
			...
			if (status.equals("Unchecked")) {
				...
				if(account < totalPrice) {
					throw new CustomException("-1", "insufficient account balance");
				}
