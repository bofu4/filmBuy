...
public interface GoodsInfoDao extends Mapper<GoodsInfo> {
    ...
    @Select("select * from goods_info where recommend = 'yes' order by id desc")
    ...
    @Select("select * from goods_info g where g.name like concat('%', #{text}, '%') or g.countrys like concat('%', #{text}, '%') or g.typeNames like concat('%', #{text}, '%') or g.beginTime like concat('%', #{text}, '%') limit 8")
